...
{
    bool nxtlev[symnum];
    memcpy(fname, "test.pl0", al);
    printf("test.pl0 is active now,\nyou can change it in main function\n\n\n");
    fin = fopen(fname, "r");
...
int block(int lev, int tx, bool *fsys)
{
    ...
    table[tx].NumOfFp = TotalNumOfFp;
    table[tx].size = dx + TotalNumOfFp;
    int j;
    for(j = 0; j < TotalNumOfFp; j++)
    {
        tx++;
        strcpy(table[tx].name, fp[j].id);
    ...
    if (tableswitch)        /* 输出名字表 */
    {
        ...
        for (i = tx0 + 1; i <= tx; i++)
        {
            ...
            case constant:
                ...
                printf("val=%d size=%d addr=%d\n", table[i].val, table[i].size, table[i].adr);
            ...
            else
            {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (table[i].kind == procedur)
                            {
                                ...
                                if(k != table[i].NumOfFp)
                                {
                                    printf("参数个数不对\n");
                                    error(51);
                                }
                                gendo(opr, table[i].NumOfFp, 17);  //没有找到对应的助记符，借用opr17
                ...
                else
                {
                    if (sym == ifsym)   /* 准备按照if语句处理 */
                    {
                        ...
                        code[cx1].a = cx;   /* 经statement处理后，cx为else后语句执行完的位置，它正是前面未定的跳转地址 */
                    }
                    else
                    {
